...
const TabletStudioContent: React.FC = () => {
  ...
  // Running natively in Electron Desktop App
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (hubParam) {
        ...
        // Always prioritize remote_pc studio camera when hub is supplied in URL
  ...
  const loadEventGallery = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/gallery/event/${selectedEventId}`);
      if (res.ok) {
  ...
  useEffect(() => {
    if (phase === 'kiosk' || showEventGalleryModal) {
      if (showEventGalleryModal) {
        setGalleryFilter('all');
      }
      loadEventGallery();
    }
  }, [phase, showEventGalleryModal, loadEventGallery]);
  ...
  useEffect(() => {
    const fetchCloudData = async () => {
      try {
        const evRes = await fetch(`${API_BASE_URL}/api/vendor/events`, { headers: { Authorization: `Bearer ${localStorage.getItem('mb_license_token') || ''}` } });
        if (evRes.ok) {
      ...
      try {
        const tmplRes = await fetch(`${API_BASE_URL}/api/vendor/templates`, { headers: { Authorization: `Bearer ${localStorage.getItem('mb_license_token') || ''}` } });
        if (tmplRes.ok) {
  ...
  const refreshCameraList = useCallback(async () => {
    try {
      ...
      const classified: DiscoveredCamera[] = videoDevices.map((dev, idx) => {
        ...
        const hasRealId = Boolean(dev.deviceId && dev.deviceId.trim().length > 0);
        return {
          deviceId: hasRealId ? dev.deviceId : 'local_webcam_default',
          label: dev.label || `Kamera ${idx + 1} (${hasRealId ? 'Tersedia' : 'Webcam Bawaan'})`,
          type,
        };
      });
      ...
      if (classified.length > 0) {
        setSelectedCameraId((prev) => {
          // Preserve remote_pc if already selected or preferred
    ...
    async (targetCameraId?: string) => {
      ...
      try {
        ...
        try {
          ...
          console.warn('Primary constraint failed, falling back to generic video:', constraintErr);
  ...
  // Keyboard Shortcut: Press ESC (closes gallery/preview modal first, or exits Kiosk/Review to Setup)
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Escape') {
        if (previewGalleryPhoto) {
  ...
  }, [phase, showEventGalleryModal, previewGalleryPhoto]);
      ...
      <div className="flex flex-col h-full w-full bg-[#0A0A0C] text-[#EDEDED] overflow-y-auto font-sans">
        ...
        <header className="h-16 px-6 border-b border-white/[0.06] flex items-center justify-between flex-shrink-0 bg-[#0F1014]">
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              onClick={() => {
                if (confirm('Keluar dari akun Studio? Anda perlu login dan aktivasi kembali perangkat ini.')) {
            ...
            >
              ...
              <span className="hidden sm:inline text-[11px] font-medium">Keluar</span>
            </button>
          </div>
        </header>
            ...
            <div className="rounded-2xl bg-[#121318] border border-white/[0.06] p-5 flex flex-col gap-4 shadow-xl">
              ...
              >
                ...
                {selectedCameraId !== 'remote_pc' && cameraError && (
                  <div className="absolute inset-0 bg-black/90 p-4 flex flex-col items-center justify-center text-center gap-2.5 text-xs text-rose-300">
      ...
      >
        ...
        >
          ...
          <button
            ...
            className="h-10 px-3.5 rounded-full bg-black/60 hover:bg-black/85 backdrop-blur-md border border-white/25 flex items-center gap-2 text-white/90 hover:text-white text-xs font-semibold shadow-2xl active:scale-95 transition-all hover:border-white/40 cursor-pointer"
